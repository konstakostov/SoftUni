...
from main_app.models import Book, Author, Artist, Song, Product, Review, Driver, DrivingLicense, Registration, Car, \
    Owner
...
def get_drivers_with_expired_licenses(due_date):
    licenses_expiration_dates = due_date - timedelta(days=365)

    expired_drivers = Driver.objects.filter(
        drivinglicense__issue_date__gt=licenses_expiration_dates
    )
